...
public class PayService {
		...
		activity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (PAY.getMoney(payPoint) > 0) {
					final int payCount = gameInfo
							.getData(GameInfoUtil.PAY_COUNT) + 1;
					gameInfo.setData(GameInfoUtil.PAY_COUNT, payCount);
					setPayInfo("request", payPoint, playerId,
							PAY.getMoney(payPoint), "0", PAY.getDesc(payPoint),
							PAY.getDesc(payPoint), payCount, orderId, "100",
							"请求支付");

					GamePay.getInstance().pay(activity, payPoint, reviveNum,
							orderId, new GamePayCallback() {
								...
								public void result(OrderResultInfo result) {
									...
									if (result.getResultCode() == 0) {
										JniPayCallbackHelper.payCallback(
												payPoint, 0);
										// TbuCloud.markUserPay(activity, 1);
										gameInfo.setData(
												GameInfoUtil.PAY_MONEY,
												gameInfo.getData(GameInfoUtil.PAY_MONEY)
														+ PAY.getMoney(payPoint));
										setPayInfo("success", payPoint,
												playerId,
												PAY.getMoney(payPoint), "0",
												PAY.getName(payPoint),
												PAY.getDesc(payPoint),
												payCount, orderId, "0", "支付成功");
									} else if (result.getResultCode() == -3) {
										JniPayCallbackHelper.payCallback(
												payPoint, 1);
										setPayInfo("cancel", payPoint,
												playerId,
												PAY.getMoney(payPoint), "0",
												PAY.getName(payPoint),
												PAY.getDesc(payPoint),
												payCount, orderId, "-3", "取消支付");
									} else {
										JniPayCallbackHelper.payCallback(
												payPoint, 1);
										setPayInfo("fail", payPoint, playerId,
												PAY.getMoney(payPoint), "0",
												PAY.getName(payPoint),
												PAY.getDesc(payPoint),
												payCount, orderId,
								...
								@Override
								public void clickOk() {
									setPayInfo("clickOk", payPoint, playerId,
											PAY.getMoney(payPoint), "0",
											PAY.getName(payPoint),
											PAY.getDesc(payPoint), payCount,
											orderId, "101", "点击确定");
								}
							});
				} else {
	...
	private static void setPayInfo(String payState, int payPoint,
		...
		TbuCloud.setPayInfo(
				...
				String.valueOf(gameInfo.getData(GameInfoUtil.USER_TYPE) == 0 ? "new"
						: "old"), new TbuCallback() {
					@Override
